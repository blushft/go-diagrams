...
func (g *Group) Attributes() attr.Attributes {
	return g.attrs
}
...
func (g *Group) Connect(start, end *node.Node, attrs ...attr.Attribute) *Group {
	g.AddNodes(start, end)
	eid := strings.SplitAfter(strings.Replace(uuid.New().String(), "-", "", 1), "-")[0]
	g.edges[eid] = edge.New(eid, start, end, attrs...)

	return g
}

func (g *Group) ConnectAllFrom(start *node.Node, attrs ...attr.Attribute) *Group {
	for id, n := range g.nodes {
		if id == start.ID() {
			continue
		}

		g.Connect(start, n, attrs...)
	}

	return g
}

func (g *Group) ConnectAllTo(end *node.Node, attrs ...attr.Attribute) *Group {
	for id, n := range g.nodes {
		if id == end.ID() {
			continue
		}

		g.Connect(n, end, attrs...)
	}

	return g
}

func (g *Group) Group(id string, attrs ...attr.Attribute) *Group {
	cg := new(id, Subgraph, defaultGroupAttrs(attrs...))
	cg.parent = g
	g.children[id] = cg
...
func (g *Group) Label(lbl string) *Group {
	attr.Label(lbl).Apply(g.attrs)
